...
async function migrateUsers() {
    ...
    for (const row of rows) {
        try {
            await User.findOneAndUpdate({ uid: row.uid }, {
                sqlId: row.id,
                uid: row.uid,
                email: row.email,
...
async function migrateLoans() {
    ...
    for (const row of rows) {
        try {
            await Loan.findOneAndUpdate({ sqlId: row.id }, {
                sqlId: row.id,
...
async function migratePayments() {
    ...
    for (const row of rows) {
        try {
            await Payment.findOneAndUpdate({ transaction_id: row.transaction_id }, {
                sqlId: row.id,
...
async function migrateDisbursements() {
    ...
    try {
        const [rows] = await db.execute('SELECT * FROM disbursements');
        for (const row of rows) {
            try {
                await Disbursement.findOneAndUpdate({ sqlId: row.id }, {
                    sqlId: row.id,
                    student_uid: row.student_uid,
...
async function migrateApplications() {
    ...
    for (const row of rows) {
        try {
            await Application.findOneAndUpdate({ sqlId: row.id }, {
                sqlId: row.id,
                student_uid: row.student_uid,
                type: row.type,
...
async function migrateChats() {
    ...
    for (const row of rows) {
        try {
            await Chat.findOneAndUpdate({ chat_id: row.chat_id }, {
                sqlId: row.id,
                chat_id: row.chat_id,
...
async function migrateEvents() {
    ...
    for (const row of rows) {
        try {
            await Event.findOneAndUpdate({ sqlId: row.id }, {
                sqlId: row.id,
                title: row.title,
...
async function migrateFootprints() {
    ...
    const [rows] = await db.execute('SELECT * FROM footprints');
    for (const row of rows) {
        try {
...
async function migrateNews() {
    ...
    for (const row of rows) {
        try {
            await News.findOneAndUpdate({ sqlId: row.id }, {
                sqlId: row.id,
                title: row.title,
                content: row.content,
...
async function migrateSupportRequests() {
    ...
    const [rows] = await db.execute('SELECT * FROM support_requests');
    for (const row of rows) {
        try {
            await SupportRequest.findOneAndUpdate({ sqlId: row.id }, {
                sqlId: row.id,
                student_uid: row.student_uid,
