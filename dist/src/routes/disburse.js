...
router.get('/:id/receipt', authenticate, async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`SELECT * FROM disbursements WHERE id = ? AND student_uid = ? LIMIT 1`, [id, uid]);
        if (!rows.length) {
...
router.get('/mine', authenticate, async (req, res) => {
    try {
        ...
        if (!uid)
            return res.status(401).json({ error: 'unauthorized' });
        const [rows] = await db.execute(`SELECT id, original_amount, deduction_amount, net_amount, approved_at, approved_by
       FROM disbursements WHERE student_uid = ? ORDER BY approved_at DESC`, [uid]);
        res.json(rows);
    }
    catch (err) {
        console.error(err);
