...
router.get('/mine', authenticate, async (req, res) => {
    try {
        ...
        const [rows] = await db.execute('SELECT * FROM applications WHERE student_uid = ? ORDER BY created_at DESC', [uid]);
        res.json(rows);
    }
    catch (err) {
...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        await db.execute('INSERT INTO applications (student_uid, type, payload, status) VALUES (?, ?, ?, ?)', [uid, type, JSON.stringify(payload), 'pending']);
