...
const ensureColumn = async (table, column, definition) => {
    const [rows] = await db.execute(`SELECT COLUMN_NAME FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = ? AND column_name = ?`, [table, column]);
    if (!rows.length) {
...
const createNotification = async (targetUid, title, message) => {
    if (!targetUid)
        return;
    try {
        await db.execute('INSERT INTO notifications (target_uid, title, message) VALUES (?, ?, ?)', [targetUid, title, message]);
    }
    catch (err) {
...
router.post('/initiate', authenticate, async (req, res) => {
    try {
        ...
        await db.execute('INSERT INTO payments (transaction_id, loan_id, user_id, amount, status, payer_uid, access_number) VALUES (?, ?, ?, ?, ?, ?, ?)', [transactionId, loanId, userId, amount, 'PENDING', req.user?.uid || null, phone]);
...
router.post('/callback', async (req, res) => {
    ...
    try {
        if (status === 'SUCCESSFUL') {
            ...
            const [paymentRows] = await db.execute("SELECT *, loan_id AS loan_resolved FROM payments WHERE transaction_id = ?", [transactionId]);
            ...
            if (payment && payment.status === 'PENDING') {
                ...
                await createNotification(payment.payer_uid, 'Payment received', `Payment of UGX ${Number(payment.amount || 0).toLocaleString()} was applied to your loan #${payment.loan_resolved}.`);
...
router.get('/loan/:loanId', authenticate, async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`SELECT p.*, p.loan_id AS loan_resolved
...
router.post('/confirm', authenticate, async (req, res) => {
    try {
        ...
        const [rows] = await db.execute(`SELECT p.id, p.amount, p.loan_id, p.status, l.id AS loan_resolved
             FROM payments p
             JOIN loans l ON p.loan_id = l.id
             WHERE p.transaction_id = ? AND p.user_id = ? LIMIT 1`, [transactionId, userId]);
        if (!rows.length) {
        ...
        await db.execute(`UPDATE payments SET status = 'SUCCESSFUL' WHERE id = ?`, [payment.id]);
        // Decrement outstanding balance on the loan
        await db.execute(`UPDATE loans SET outstanding_balance = outstanding_balance - ? WHERE id = ?`, [payment.amount, payment.loan_resolved]);
        // Notify the student that payment was applied
        await createNotification(req.user?.uid, 'Payment received', `Payment of UGX ${Number(payment.amount || 0).toLocaleString()} was applied to your loan #${payment.loan_resolved}.`);
        return res.json({
...
router.get('/mine', authenticate, async (req, res) => {
    try {
        await ensurePaymentsSchema();
        const userId = req.user.id;
        const [rows] = await db.execute(`SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC`, [userId]);
        res.json(rows);
    }
    catch (err) {
...
router.get('/:paymentId/receipt', authenticate, async (req, res) => {
    try {
        ...
        const [paymentRows] = await db.execute(`SELECT p.*, p.loan_id AS loan_resolved, u.full_name, u.email 
